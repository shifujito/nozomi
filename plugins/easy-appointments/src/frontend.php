...
<div class="ea-standard">
  <form>
    <div class="step">
      <div class="block"></div>
      <label
        class="ea-label"><?php echo esc_html(__($this->options->get_option_value("trans.location"), 'easy-appointments')) ?></label><select
        name="location" data-c="location" class="filter"><?php $this->get_options("locations") ?></select>
    </div>
    <div class="step">
      <div class="block"></div>
      <label
        class="ea-label"><?php echo esc_html(__($this->options->get_option_value("trans.service"), 'easy-appointments')) ?></label><select
    ...
    <div class="step">
      <div class="block"></div>
      <label
        class="ea-label"><?php echo esc_html(__($this->options->get_option_value("trans.worker"), 'easy-appointments')) ?></label><select
    ...
    <div class="step final">
      <div class="block"></div>
      <p class="section">お客様情報を入力してください。</p>
      <br>
      ...
      <p>
        <label style="font-size: 65%; width: 80%;"
          class="i-agree"><?php _e('I agree with terms and conditions', 'easy-appointments'); ?>
          * : </label><input style="width: 15%;" type="checkbox" name="iagree" data-rule-required="true"
      ...
      <p>
        <label style="font-size: 65%; width: 80%;" class="gdpr"><?php echo esc_html($settings['gdpr.label']);?>
          * : </label><input style="width: 15%;" type="checkbox" name="iagree" data-rule-required="true"
      ...
      <div style="width: 100%" class="g-recaptcha" data-sitekey="<?php echo esc_attr($settings['captcha.site-key']);?>">
      </div><br>
      <?php endif; ?>
...
<?php
    ...
    public function ea_bootstrap($atts)
    {
        ...
        $settings['block_days_tooltip']     = $code_params['block_days_tooltip'];
            ...
            // LOCALIZATION
        $settings['trans.please-select-new-date'] = __('Please select another day', 'easy-appointments');
        $settings['trans.personal-informations'] = __('お客様情報', 'easy-appointments');
        ...
        $settings['trans.booking-overview'] = __('予約情報', 'easy-appointments');
        $settings['trans.date-time'] = __('予約日時', 'easy-appointments');
        $settings['trans.submit'] = __('予約', 'easy-appointments');
        $settings['trans.cancel'] = __('キャンセル', 'easy-appointments');
        $settings['trans.price'] = __('Price', 'easy-appointments');
...
wp_enqueue_script('ea-google-recaptcha-v3',
"https://www.google.com/recaptcha/api.js?render={$settings['captcha3.site-key']}");
}
